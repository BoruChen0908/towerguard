...
class TestXadd:
    def test_xadd_writes_four_contract_fields(self, fake_redis):
        xadd_shift_event(
            fake_redis,
        ...
        assert set(fields.keys()) == {"timestamp", "kind", "summary", "ref"}
...
class TestNullRef:
    ...
    def test_decode_entry_coerces_bytes(self):
        decoded = decode_entry(
            {b"timestamp": b"t", b"kind": b"advisory", b"summary": b"s", b"ref": b"R1"}
        )
        assert decoded == {
            "timestamp": "t",
            "kind": "advisory",
